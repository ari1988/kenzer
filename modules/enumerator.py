...
class Enumerator:
    ...
    def httpx(self, domains, output, extras=""):
        os.system(
            "httpx {2} -no-color -l {0} -rl 70 -threads 70 -retries 2 -timeout 8 -verbose -o {1}".format(domains, output, extras))
        return
    ...
    def katana(self):
        ...
        if(os.path.exists(path)):
            os.system("rm {0}".format(path))
        os.system("katana -list {0}/webenum.kenz -o {1} -jc -kf -sc -retry 2 -d 3 -aff -c 25 -p 15 -fs fqdn".format(self.path, path))
        return
